...
class TypeResolver {
    ...
    [[nodiscard]] OptId<TypeId> type_anon_struct(FileId fid, ScopeId scope, const ast_type_t* type,
                                                 bool need_layout_info) {
        assert(type->tag == AST_TYPE_ANON_STRUCT);
        llvm::SmallVector<TypeId> tid_vec{};
        const auto ty_slice = type->type.anon_struct.types;
        for (size_t i = 0; i < ty_slice.len; ++i) {
            const auto maybe_tid = resolve_type(fid, scope, ty_slice.start[i], need_layout_info);
            if (maybe_tid.empty()) {
                return {};
            }
            tid_vec.push_back(maybe_tid.as_id());
  ...
  public:
    ...
    [[nodiscard]] OptId<TypeId> resolve_type(FileId fid, ScopeId scope, const ast_type_t* type,
        ...
        case AST_TYPE_ANON_STRUCT:
            return type_anon_struct(fid, scope, type, need_layout_info);
