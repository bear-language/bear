...
[[nodiscard]] LayoutId Context::emplace_layout(Layout lay) {
    return layouts.emplace_and_get_id(lay);
}
...
[[nodiscard]] OptId<DefId> Context::look_up_member_function_guarding_hid(DefVisitor& def_visitor,
                                                                         const Def& struct_def,
                                                                         SymbolId symbol_id,
                                                                         Span id_span,
                                                                         ScopeId local_scope) {
...
[[nodiscard]] OptId<DefId> Context::look_up_generic_member_function_guarding_hid(
...
bool Context::is_struct(DefId did) const {
    const auto* node = def_ast_node(did);
    if (!node) {
        return false;
    }
    return node->type == AST_STMT_STRUCT_DEF;
}
/// checks if a Def is a struct without resolving it
...
bool Context::is_variant(DefId did) const {
    ...
    return node->type == AST_STMT_VARIANT_DEF;
}

bool Context::is_function(DefId did) const {
    const auto* node = def_ast_node(did);
    if (!node) {
        return false;
    }
    return node->type == AST_STMT_FN_DECL;
}
...
bool Context::is_variant_field(DefId did) const {
    ...
    return node->type == AST_STMT_VARIANT_FIELD_DECL;
}
...
bool Context::is_contract(DefId did) const {
    ...
    return node->type == AST_STMT_CONTRACT_DEF;
}
...
OptId<DefId> Context::try_def_for_type(TypeId tid) const {
    const Type& t = type(tid);
    if (t.holds<TypeStruct>()) {
        return t.as<TypeStruct>().def_id;
    }
    if (t.holds<TypeVariant>()) {
...
[[nodiscard]] OptId<DefId> Context::try_generic_instantiation(DefVisitor& def_visitor,
                                                              DefId orig_def_id,
...
[[nodiscard]] bool Context::validate_gen_args_for_def(DefVisitor& def_visitor, DefId did,
                                                      GenericArgIdSliceId gen_args_id) {
    if (!def(did).generic) {
        const Def& d = def(did);
    ...
    DiagLinker dl{*this};
    auto validate_arg = [this, &dl, &def_visitor](GenericArgId aid, GenericParamId pid) -> bool {
        Ovld vs{
            [this, pid, &dl, &def_visitor](TypeId tid) -> bool {
                ...
                if (param.holds<GenericParamType>()) {
                    ...
                    for (auto didx = contracts.begin(); didx != contracts.end(); ++didx) {
                        ...
                        if (!type_has_contract(tid, def_visitor.visit_as_dependent(contract_did))) {
                            dl.link(emplace_diagnostic_with_message_value(
                                type(tid).span, diag_code::does_not_have_contract, diag_type::error,
                    ...
                    if (cooked) {
                        dl.link(emplace_diagnostic_with_message_value(
                            param.span, diag_code::declared_here_as_constrained_generic_parameter,
...
[[nodiscard]] OptId<DefId> Context::try_instantiate_def_on_scoped_lookup_if_needed(
    ...
    if (!maybe_gen_args.has_value() || maybe_gen_args.value().is_empty()) {
        ...
        const Def& d = def(orig_did);
        Span span = arg.holds<ExecId>() ? exec(arg.as<ExecId>()).span : type(arg.as<TypeId>()).span;
        auto d0 = emplace_diagnostic_with_message_value(
...
OptId<TypeId> Context::do_type_inference_from_exec(ExecId eid) {
    ...
    auto vs = Ovld{
        ...
        [this](const ExecVariantInit& d) -> OptId<TypeId> {
            const auto did = d.variant_def_id;
        ...
        [this](const ExecStructInit& d) -> OptId<TypeId> {
            auto struct_did = d.struct_def_id;
        ...
        [this, &exec](const ExecListLiteral& d) -> OptId<TypeId> {
            auto maybe_contained_type = d.elem_type_id;
        ...
        [this](const ExecRange& d) -> OptId<TypeId> {
            DefVisitor def_visitor{*this};
            const GenericArgIdSliceId gargs
                = emplace_generic_arg_id_slice(freeze_id_vec(llvm::SmallVector<GenericArgId, 1>{
                    emplace_generic_arg(infer_type_from_exec(d.start).as_id())}));

            const OptId<DefId> range_def_id
                = look_up_scoped_type_generic(def_visitor, root_scope(),
                                              freeze_id_vec(llvm::SmallVector<SymbolId, 2>{
                                                  symbol_id<"std">(), symbol_id<"Range">()}),
        ...
        [this](const ExecFnCall& d) -> OptId<TypeId> {
            ...
            const Type& ty = type(maybe_tid.as_id());
            if (ty.holds<TypeFnPtr>()) {
                return ty.as<TypeFnPtr>().return_type;
            }
            return {};
        },
        ...
        [](const ExecBlock&) -> OptId<TypeId> { return {}; },
        [](const ExecBranch&) -> OptId<TypeId> { return {}; },
        [](const ExecReturn&) -> OptId<TypeId> { return {}; },
        [](const ExecYield&) -> OptId<TypeId> { return {}; },
        [](const ExecJump&) -> OptId<TypeId> { return {}; },
    };

    return exec.visit(vs);
}
