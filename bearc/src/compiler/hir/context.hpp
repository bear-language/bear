...
class Context {
  public:
    ...
    [[nodiscard]] OptId<DefId>
    look_up_generic_member_function_guarding_hid(DefVisitor& def_visitor, const Def& struct_def,
                                                 SymbolId symbol_id, Span id_span,
    ...
    // gets inner tid if tid corresponds to a TypeRef, this also bypasses deftypes
    [[nodiscard]] TypeId try_decay(TypeId tid) const;
  ...
  private:
    ...
    DataArena canon_tid_to_offset_slice_arena;
    IdHashMap<CanonicalTypeId, OffsetSliceId> canon_tid_to_offset_slice;
